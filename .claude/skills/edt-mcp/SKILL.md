...
EDT_MCP — это MCP-плагин для 1C:EDT, экспортирующий **99 инструментов** работы с проектами 1С:Предприятие через HTTP+SSE. Этот скилл — практический справочник: реальные имена параметров, рецепты для типовых задач, главные правила работы.
...
**Полная таблица всех 99 инструментов с точными именами аргументов и тип-выражениями —
...
## Что НЕ работает / не покрыто (по состоянию на v1.21.0)

Сверено с кодом main v1.21.0 (2026-08-02). Часть граблей прошлых редакций **закрыта** — см. «Починено в v1.18.0–v1.21.0» ниже, не воспроизводи старые workaround'ы. Оставшиеся пункты — ограничения платформы 1С/EDT, кодом плагина не закрываются.
...
### Починено в v1.18.0–v1.21.0 — старые workaround'ы больше не нужны
