...
EDT_MCP — это MCP-плагин для 1C:EDT, экспортирующий **100 инструментов** работы с проектами 1С:Предприятие через HTTP+SSE. Этот скилл — практический справочник: реальные имена параметров, рецепты для типовых задач, главные правила работы.
...
1. **Сервер должен крутиться в EDT IDE.** Инстанция занимает первый свободный порт диапазона (по умолчанию 3001–3006), поэтому проверять надо диапазон: `Get-NetTCPConnection -State Listen | Where-Object LocalPort -in 3001..3006`. Точный адрес живой инстанции — в её маячке `~/.edt-mcp/instances/<pid>.json` (поля `port`, `workspacePath`, `projects`; JSON начинается со смещения 1, нулевой байт занят замком присутствия). Автозапуск по умолчанию **выключен**: если сервера нет, пользователь включает Auto-start в `Window → Preferences → EDT MCP` либо жмёт `Window → EDT MCP → Start`.
2. **Токен** — в `Window → Preferences → EDT MCP` (одна строка). Сохрани в файл, скажем `MCP_token.txt`.
3. **Endpoints** (порт — фактический, из маячка): SSE = `http://127.0.0.1:<порт>/mcp/sse`, messages = `http://127.0.0.1:<порт>/mcp/messages?sessionId=…` (sessionId возвращает SSE-handshake в первом `event: endpoint`).
4. **Тестировать через готовый harness**: [references/smoke-harness.md](references/smoke-harness.md).

### Несколько инстанций EDT одновременно (с 1.23.0)

Рабочая область Eclipse процессная: сервер видит только проекты своего workspace.
Инстанций может быть несколько, каждая на своём порту диапазона. Прежде чем
работать — определи нужную: прочитай `~/.edt-mcp/instances/*.json`, найди запись,
у которой требуемый проект есть в `projects[]`, и дальше обращайся на её `port`.
Подтверди выбор одним вызовом `get_workspace_info` (он отдаёт `root`, `projects`,
`port`, `pid`); сервер и сам представляется в `initialize` — `serverInfo.title`
вида `EDT_MCP @ <workspace>` и первый абзац `instructions`.

Монопольные операции (`deploy_project`, прогоны тестов, сборка и импорт внешних
объектов, отладка, `create_infobase`) берут межпроцессный замок: если ресурс занят
соседней инстанцией, вернётся отказ с именем держателя и pid — это не баг, а
...
**Полная таблица всех 100 инструментов с точными именами аргументов и тип-выражениями —
...
Группировка: workspace/projects (8), infobase+deploy (5), metadata (34), eventlog (2),
...
- **`import_external_object`** (v1.22.0) заводит **готовый** `.epf`/`.erf` в существующий external-object проект — тем же путём, что мастер «Импорт → Внешние отчёты и обработки» в IDE (`IExternalObjectRestorer` распаковывает файл в Designer-XML, `IImportOperationFactory` импортирует его в исходники EDT). Аргументы: `project*`, `file*`, `overwrite`, `timeoutSeconds`. Версия платформы и базовый проект конфигурации берутся из самого проекта, ИБ для распаковки — из ассоциированного приложения (`associate_infobase` обязателен, ИБ блокируется на время). **Имя объекта определяется содержимым файла, а не его именем**: `АРМ_150626.epf` даёт `ExternalDataProcessor.АРМ` — переименовать потом можно через `rename_md_object`. Существующий объект с тем же именем не перезаписывается, пока не передан `overwrite=true`.
...
Если в EDT уже есть настроенная конфигурация запуска — предпочитай `run_launch_configuration`
(1.21.0): она запускает тем же кодом, что кнопка в IDE, с учёткой/типом клиента/версией
платформы из самой конфигурации, и не страдает от расхождений ручной командной строки 1cv8
(кодировка кириллического логина, версия ИБ в свойствах и т.п.).

```jsonc
[
  { "tool": "list_launch_configurations", "args": {} },
  { "tool": "run_launch_configuration", "args": { "name": "Upiter Тонкий клиент", "mode": "run" } },
]
```

`run_client` — низкоуровневый вариант, когда конфигурации запуска нет:
...
## Что НЕ работает / не покрыто (по состоянию на v1.23.0)

Сверено с кодом main v1.23.0 (2026-08-18). Часть граблей прошлых редакций **закрыта** — см. «Починено в v1.18.0–v1.22.0» ниже, не воспроизводи старые workaround'ы. Оставшиеся пункты — ограничения платформы 1С/EDT, кодом плагина не закрываются.
...
### Починено в v1.18.0–v1.22.0 — старые workaround'ы больше не нужны

- **Импорт готового `.epf`/`.erf` в проект (v1.22.0):** `import_external_object` — раньше файл
  заводился в EDT только руками через мастер импорта в IDE. Теперь цикл «взять существующую
  обработку → править исходники в EDT → `build_external_object`» замыкается без GUI.
- **Запуск клиента через launch-конфигурации EDT (v1.21.0):** `list_launch_configurations` +
  `run_launch_configuration` — запуск тем же кодом, что кнопка run/debug в IDE (учётка, тип
  клиента, версия платформы из конфигурации). Закрывает класс проблем «headless 1cv8 не
  принимает учётку, которая работает из EDT» и «run_client не находит установку по версии
  из свойств ИБ».

- **`create_md_object` создаёт все data-kinds (v1.20.0, донастройка — v1.20.1):** `Task`, `BusinessProcess`, `ChartOfAccounts`, `ChartOfCalculationTypes`, `ChartOfCharacteristicTypes`, `ExchangePlan`, `DocumentJournal` больше не отбиваются. Донастройка после создания: бизнес-процессу задай задачу (`set_md_property property=task value=Task.X` — иначе error «Не выбрана задача»), журналу — регистрируемые документы (`add_register_recorder register=DocumentJournal.X document=Document.Y` — иначе error «не заданы регистрируемые документы»); `ExchangePlan` получает `thisNode` автоматически.
