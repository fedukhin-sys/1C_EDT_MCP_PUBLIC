...
- **Запрос командных строк из 1.23.2 не работал.** Сам механизм был верен, но скрипт
  передавался интерпретатору аргументом `-Command`, а он содержит кавычки и скобки:
  склейка аргументов на Windows кавычки внутри значения не экранирует, интерпретатор
  получал искажённую строку и молча отдавал пустой вывод. Внешне всё выглядело
  исправным — ошибки нет, просто поле `infobase` у чужих процессов снова пустое.
  Теперь команда уходит в Base64 (`-EncodedCommand`), где кавычек нет как класса.
  Проверено на трёх запущенных инстанциях: базы `localhost\UH-KA`, `localhost\RB`
  и `localhost\Dandy` распознаются.
- **Закрыт пробел в тестах, который это пропустил.** Прежние тесты подменяли источник
  данных и настоящий запрос никогда не выполняли. Добавлены проверка кодирования
  (Base64 от UTF-16LE с обратным разбором) и живой прогон самого запроса.

## [1.23.2] — 2026-08-18

### Исправлено

- **`list_running_clients` не называл базу чужого процесса.** С `includeForeign: true`
  инструмент показывал, что процесс платформы держит информационную базу, но само
  поле `infobase` всегда оставалось пустым — а в нём весь смысл: осиротевший `1cv8`
  превращает `deploy_project` в тихий no-op, и надо знать, какую именно базу он
  занял. Причина уровнем выше разбора: на Windows `ProcessHandle` не отдаёт
  командную строку **чужого** процесса, хотя имя образа выдаёт. Теперь при пустой
  командной строке подключается разовый запрос к CIM — по одному на весь скан и
  только если есть что дозаполнять. Вне Windows и при любой осечке поле остаётся
  пустым, как и раньше; процессы, которые не отдают командную строку и через CIM,
  честно остаются неизвестными — это ограничение операционной системы.
...
[1.23.3]: https://github.com/fedukhin-sys/1C_EDT_MCP_PUBLIC/releases/tag/v1.23.3
[1.23.2]: https://github.com/fedukhin-sys/1C_EDT_MCP_PUBLIC/releases/tag/v1.23.2
