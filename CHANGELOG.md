...
сообщением «распаковка не дала корневого XML» — искал результат распаковки не там,
где его оставляет платформа.

### Исправлено

- **Корень выгрузки — сам временный каталог.** Дерево (`Ext/`, `Forms/`,
  `Templates/`) платформа кладёт внутрь него, а описание обработки — файлом
  `<каталог>.xml` рядом, уровнем выше. Инструмент искал этот файл внутри каталога,
  где его нет.
- **Песочница агента конфигуратора.** Для серверной информационной базы 1C:EDT
  запускает платформу через агент (`/AgentMode`), прокидывая файлы в
  `%TEMP%\1cedt\ssh-*\0\`. Обратно оттуда забирается только каталог результата, а
  `<каталог>.xml` остаётся в песочнице — распакованное дерево приезжало без описания
  обработки. Теперь файл забирается и оттуда. Мастер «Импорт → Внешние отчёты и
  обработки» в IDE этой развилки не знает и на серверной базе спотыкается так же.
