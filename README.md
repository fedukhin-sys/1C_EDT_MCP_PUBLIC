...
MCP-сервер для 1C:EDT — **v1.23.0** (workspace, проекты, модули BSL, инфобазы, деплой, запуск клиента, отладка, проверки качества, редактирование метаданных, авторство форм, xUnitFor1C, схемы СКД, журнал регистрации, обезличивание ПДн 152-ФЗ).
...
- **Port (range start)** — начало диапазона портов SSE (по умолчанию `3001`);
- **Port range end** — конец диапазона включительно (по умолчанию `3006`). Инстанция при старте занимает первый свободный порт диапазона, поэтому несколько одновременно запущенных 1C:EDT не мешают друг другу. Чтобы закрепить фиксированный порт, задайте конец диапазона равным началу;
...
Клик по статус-итему открывает страницу Preferences. В меню `Window → EDT MCP` доступны явные команды **Start / Stop / Restart**. Смена порта или конца диапазона в Preferences автоматически перезапускает сервер.
...
## Несколько инстанций 1C:EDT одновременно

Рабочая область Eclipse — величина процессная: сервер одной инстанции видит
только свои проекты. Поэтому работа с несколькими конфигурациями сразу — это
несколько запущенных 1C:EDT, у каждой свой MCP-сервер на своём порту.

**Порты разводятся сами.** Настройка задаёт диапазон (по умолчанию 3001–3006),
при старте инстанция занимает первый свободный. До 1.23.0 порт был один, и
вторая инстанция молча оставалась без MCP-сервера, упав `BindException`.

**Как найти нужную.** Каждая живая инстанция пишет
`~/.edt-mcp/instances/<pid>.json`:

```json
{ "pid": 13204, "port": 3002, "bindHost": "127.0.0.1",
  "sseUrl": "http://127.0.0.1:3002/mcp/sse",
  "workspacePath": "E:/Projects/Demo", "workspaceName": "Demo",
  "projects": ["Demo"], "version": "0.1.0.202608180711", "startedAt": "2026-08-18T05:42:00Z" }
```

Поле `version` — версия бандла `ru.fedukhin.edt.mcp.core` с квалификатором сборки,
а не номер релиза плагина: они живут разными циклами.

Читать файл нужно **со смещения 1**: нулевой байт занят замком присутствия и у
живой инстанции на Windows недоступен для чтения — `JSON.parse` всего файла упадёт.

Файл существует, пока жив процесс: на нём удерживается блокировка, которую ОС
снимает при крахе, поэтому осиротевшая запись отличается от живой механически —
без эвристик по PID. Клиент читает каталог и по имени проекта выбирает порт.

**Конфигурация клиента.** Объявите слоты один раз, по одному на порт диапазона:

```jsonc
{ "mcpServers": {
    "edt-1": { "type": "sse", "url": "http://127.0.0.1:3001/mcp/sse",
               "headers": { "Authorization": "Bearer <token>" }, "timeout": 1800000 },
    "edt-2": { "type": "sse", "url": "http://127.0.0.1:3002/mcp/sse", "…": "…" }
} }
```

Слот — это порт, а не проект: соответствие плавающее, новый проект не требует
правки конфига. Токен один на всех — он хранится per-user, а не per-workspace.
Неподнятые слоты ничего не стоят: инструменты приходят только от подключённых
серверов. Поле `timeout` обязательно, иначе долгий `deploy_project`
упрётся в пятиминутное idle-окно клиента.

**Как сервер сообщает, кто он.** В ответе `initialize` приходит
`serverInfo.title` вида `EDT_MCP @ Demo`, а первый абзац `instructions` называет
рабочую область и открытые проекты. Проверить после подключения —
`get_workspace_info`: он отдаёт `root`, `projects`, `port` и `pid`.

**Что защищено от гонок.** Монопольные операции берут межпроцессный замок, и
ключ подбирается под ресурс, за который идёт борьба: информационная база —
`deploy_project`, `run_tests`, `run_test_method`; проект и выходной файл —
сборка и импорт внешних объектов; порт сервера отладки — `debug_client`; список
информационных баз — `create_infobase`. Отказ называет держателя (инструмент и
pid, а `deploy_project` — ещё и рабочую область), а не безликий таймаут.
Дополнительно `deploy_project` и прогоны тестов сверяют запрошенную базу с
ассоциацией проекта и отказываются работать с чужой без явного
`allowForeignInfobase: true` — список баз в 1С один на пользователя, и до 1.23.0
одна опечатка уводила расширение в чужую базу без единой ошибки.
