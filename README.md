...
MCP-сервер для 1C:EDT — **v1.23.3** (workspace, проекты, модули BSL, инфобазы, деплой, запуск клиента, отладка, проверки качества, редактирование метаданных, авторство форм, xUnitFor1C, схемы СКД, журнал регистрации, обезличивание ПДн 152-ФЗ).
...
{ "pid": 13204, "port": 3002, "bindHost": "127.0.0.1",
  ...
  "projects": ["Demo"], "version": "1.23.3.202608181110", "startedAt": "2026-08-18T05:42:00Z" }
```

Поле `version` — версия ядра плагина с квалификатором сборки, который проставляет
сборка; сама версия совпадает с номером релиза.
